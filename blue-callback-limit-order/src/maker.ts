...
async function getOrCreateCallback(context: MakerContext, salt: Hash) {
  ...
  if (callback !== zeroAddress) return callback
  ...
  if (created === zeroAddress) throw new Error('callback creation failed')
  return created
}

async function approveBlueIfNeeded(context: MakerContext, assets: bigint) {
...
async function authorizeRatifierIfNeeded(context: MakerContext) {
...
export async function prepareBlueCallbackPosition({
  ...
  const callback = await getOrCreateCallback(context, salt)
  await approveBlueIfNeeded(context, assets)
  await supplyBlue(context, callback, assets)
  await authorizeRatifierIfNeeded(context)
  return callback
}
